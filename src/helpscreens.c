...
void helpLegend(int nextState)
{
    ...
    unsigned int tmpCrank = crankUpdate();
    if ((tmpCrank == CRANKMOVELEFT ) || (tmpCrank == CRANKMOVERIGHT) || 
...
//FINISH LEVEL STATE
void helpSlide2(void)
{
    helpFinishLevel(gsInitHelpSlide3);
}

//SLIDE STATE
void helpSlide3(void)
{
    helpDoSlideRotate(gsInitTitle);
}